...
from core.users.forms import (RegistrationForm, LoginForm, UpdateAccountForm, RequestResetForm, ResetPasswordForm)
...
# using < > in the route makes it a dynamic parameter, a string of the username clicked on is passed into the route
@users.route("/user/<string:username>")
def user_posts(username):
    page = request.args.get('page', 1, type=int) # 1 is the default page number
    user = User.query.filter_by(username=username).first_or_404()
    # using a backslash (\) can split a single line up into multiple lines
